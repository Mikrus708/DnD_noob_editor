...
#pragma warning disable CS0642 // Possible mistaken empty statement
            switch (SW)
            {
                case 1:
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 91) ;
                    else if (roll < 96) items.Add(new Item { Name = "Klejnot" });
                    else items.Add(new Item { Name = "Dzieło sztuki" });
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 72) ;
                    else if (roll < 96) items.Add(new Item { Name = "Zwykły przedmiot" });
                    else items.Add(new Item { Name = "Słaby przedmiot magiczny" });
                    break;
                case 2:
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 14) ;
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 82) ;
                    else if (roll < 96) { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 50) ;
                    else if (roll < 86) items.Add(new Item { Name = "Zwykły przedmiot" });
                    else items.Add(new Item { Name = "Słaby przedmiot magiczny" });
                    break;
                case 3:
                    roll = Dice.Roll(Dice.Type.k100);
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 50) ;
                    else if (roll < 80) { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Zwykły przedmiot" }); }
                    else items.Add(new Item { Name = "Słaby przedmiot magiczny" });
                    break;
                case 4:
                    roll = Dice.Roll(Dice.Type.k100);
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 71) ;
                    else if (roll < 96) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 43) ;
                    else if (roll < 63) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Zwykły przedmiot" }); }
                    else items.Add(new Item { Name = "Słaby przedmiot magiczny" });
                    break;
                case 5:
                    roll = Dice.Roll(Dice.Type.k100);
                    ...
                    else { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 58) ;
                    else if (roll < 68) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Zwykły przedmiot" }); }
                    else { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby przedmiot magiczny" }); }
                    break;
                case 6:
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 11) ;
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 57) ;
                    else if (roll < 93) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 55) ;
                    else if (roll < 60) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Zwykły przedmiot" }); }
                    else if (roll < 100) { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby magiczny przedmiot" }); }
                    else items.Add(new Item { Name = "Przeciętny magiczny przedmiot" });
                    break;
                case 7:
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 12) ;
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 49) ;
                    else if (roll < 89) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 52) ;
                    else if (roll < 98) { roll = Dice.Roll(Dice.Type.k3); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby magiczny przedmiot" }); }
                    else items.Add(new Item { Name = "Przeciętny magiczny przedmiot" });
                    break;
                case 8:
                    roll = Dice.Roll(Dice.Type.k100);
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 46) ;
                    else if (roll < 86) { roll = Dice.Roll(Dice.Type.k6); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 49) ;
                    else if (roll < 97) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby magiczny przedmiot" }); }
                    else items.Add(new Item { Name = "Przeciętny magiczny przedmiot" });
                    break;
                case 9:
                    roll = Dice.Roll(Dice.Type.k100);
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 41) ;
                    else if (roll < 81) { roll = Dice.Roll(Dice.Type.k8); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 44) ;
                    else if (roll < 92) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby magiczny przedmiot" }); }
                    else items.Add(new Item { Name = "Przeciętny magiczny przedmiot" });
                    break;
                case 10:
                    roll = Dice.Roll(Dice.Type.k100);
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 36) ;
                    else if (roll < 80) { roll = Dice.Roll(Dice.Type.k8); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k6); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 41) ;
                    else if (roll < 89) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby magiczny przedmiot" }); }
                    else if (roll < 100) items.Add(new Item { Name = "Przeciętny magiczny przedmiot" });
                    else items.Add(new Item { Name = "Potężny magiczny przedmiot" });
                    break;
                case 11:
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 9) ;
                    ...
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 25) ;
                    else if (roll < 75) { roll = Dice.Roll(Dice.Type.k10); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Klejnot" }); }
                    else { roll = Dice.Roll(Dice.Type.k6); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Dzieło sztuki" }); }
                    roll = Dice.Roll(Dice.Type.k100);
                    if (roll < 32) ;
                    else if (roll < 85) { roll = Dice.Roll(Dice.Type.k4); for (int i = 0; i < roll; i++) items.Add(new Item { Name = "Słaby magiczny przedmiot" }); }
                    else if (roll < 99) items.Add(new Item { Name = "Przeciętny magiczny przedmiot" });
                    else items.Add(new Item { Name = "Potężny magiczny przedmiot" });
                    break;
                default:
                    break;
